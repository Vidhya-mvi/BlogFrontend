...
const Home = () => {
  ...
  const handleLike = async (id) => {
    if (!user || !user._id) return toast.warn("Please log in to like blogs!");
  ...
  const handleComment = async (id) => {
    ...
    if (!user || !user._id) return toast.error("Please log in to comment");
    ...
    try {
      ...
      setBlogs((prev) =>
        prev.map((b) => (b._id === id ? { ...b, comments: updatedComments } : b))
      );
  ...
  if (!Array.isArray(blogs) || blogs.length === 0) return <h3 style={{ color: "black" }}>No blogs found.</h3>;
          ...
          return (
            ...
            >
              ...
              <div style={{ padding: "10px" }}>
                <h2 style={{ fontSize: "1rem", color: "#333" }}>{blog.title || "Untitled Blog"}</h2>
